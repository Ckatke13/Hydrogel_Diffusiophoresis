...
#takes velocity fields exported from strong_acid_2D.mph as input and creates streamline profiles
...
X = np.loadtxt('velocity_x_strong.txt', comments='%')[:,0] #exported from strong_acid_2D.mph
Y = np.loadtxt('velocity_x_strong.txt', comments='%')[:,1]
U = np.loadtxt('velocity_x_strong.txt', comments='%')[:,2:]
V = np.loadtxt('velocity_y_strong.txt', comments='%')[:,2:] #exported from strong_acid_2D.mph
X_sol = np.loadtxt('sol_vel_x_strong.txt', comments='%')[:,0] #exported from strong_acid_2D.mph
Y_sol = np.loadtxt('sol_vel_x_strong.txt', comments='%')[:,1]
U_sol = np.loadtxt('sol_vel_x_strong.txt', comments='%')[:,2:]
V_sol = np.loadtxt('sol_vel_y_strong.txt', comments='%')[:,2:] #exported from strong_acid_2D.mph
...
def speedplot(n):
    ...
    ax.set_xlim([0,1])
    ax.set_ylim([0,2])
    pcm = ax.pcolormesh(X_grid, Y_grid, speed_intp, shading='gouraud', cmap='YlOrBr', norm=colors.LogNorm(vmin=0.001, vmax=max_speed))
    ...
    plt.savefig(f'{n}velocity_strong.png', bbox_inches='tight', pad_inches=0,dpi=300, transparent=True)
    plt.close(fig)
    speed_intp=[]
    V_grid=[]
    U_grid=[]
...
"""def combine(n):
    img1 = plt.imread(f'{n}velocity.png')
    img2 = plt.imread(f'{n}deform.png')
    fig, ax =  plt.subplots(figsize=(12.5,5))

    cmap = mpl.cm.YlOrBr
    norm = mpl.colors.LogNorm(vmin=0.0001, vmax=0.0270851)

    cbar = fig.colorbar(mpl.cm.ScalarMappable(norm=norm, cmap=cmap), orientation='horizontal', aspect=20, shrink=0.4, pad=0.25)
    cbar.set_ticks([0.0001,0.001,0.01],labels=[r'$<10^{-4}$',r'$10^{-3}$',r'$10^{-2}$'], size=15)
    cbar.set_label(r'Fluid Speed$\times U^{(0)}$', size=15,family='sans-serif')
    ...
    ax.set_xticks(np.arange(0,8001,step=1600), ['0', '500H', '1000H', '1500H', '2000H', '2500H'])
    ...
    plt.savefig(f'{n}combined_just.png', dpi=300, pad_inches=0, bbox_inches='tight')
    plt.close(fig)"""
