##Code for modeling 1D hydrogel exposed to 1M of HCl. Ouputs are solutions for various pertinent quantities as pvd files.
...
u0 = Expression(('-0.21875*x[0]', '0.', '0.', '0.', 'phiac_hcl2*0.5*(1-tanh(500*(x[0]-0.98)))', 'phiac', '0.'), degree=1, phiac = phiac, phiac_hcl2 = phiac_hcl2)
un = interpolate(u0, V)
...
g2 = Constant(0.)
g3 = Constant(0.)
#g4 = -xi*Cudiff/(delta)*dot(grad(u2),n)
...
vtkfile_u1 = File('strong_acid_1D_varied_acid_front_ca/deform.pvd')
vtkfile_u2 = File('strong_acid_1D_varied_acid_front_ca/phi_o.pvd')
vtkfile_u3 = File('strong_acid_1D_varied_acid_front_ca/phi_o_hcl.pvd')
vtkfile_u4 = File('strong_acid_1D_varied_acid_front_ca/phi_a.pvd')
vtkfile_u5 = File('strong_acid_1D_varied_acid_front_ca/phi_a_hcl.pvd')
vtkfile_u11 = File('strong_acid_1D_varied_acid_front_ca/phi_b.pvd')
vtkfile_u12 = File('strong_acid_1D_varied_acid_front_ca/phi_b_hcl.pvd')
vtkfile_u13 = File('strong_acid_1D_varied_acid_front_ca/Pi.pvd')
vtkfile_u14 = File('strong_acid_1D_varied_acid_front_ca/du_dt.pvd')
...
time = np.array([])
du_dt = Constant(0.)
du_dt = project(du_dt,W)
du_dt.rename('du_dt','du_dt')
...
while t<T:
    ...
    if nn%100==0:
        ...
        osp = project((-grad(_u1)[0]-chi*_u7-gamma*_u6+K*eta*_u2+K*0.5*phi_p_eq*phi_p_eq), W)
        osp.rename('osp','osp')
        vtkfile_u13 << (osp,t)
    ...
    du_dt = project(((u1 + ccn*un1 + ccn_1*un_11 + ccn_2*un_21 + ccn_3*un_31 + ccn_4*un_41)/(dt*ccr)),W)
    du_dt.rename('du_dt','du_dt')
...
np.savetxt('fast_kf_4_19_K_1000_vc_1_eta_0.225_bound_calcium_ca_gel_0.6_refined_mesh_varied_acid_front.txt', flux_net_bound_copper)
np.savetxt('fast_kf_4_19_K_1000_vc_1_eta_0.225_osmotic_pressure_ca_gel_0.6_refined_mesh_varied_acid_front.txt', flux_os)
