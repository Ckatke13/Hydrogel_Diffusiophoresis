...
###This notebook creates velocity streamlines and combines it with deformation image generated from Grid_deformation_weak_9.2_figure_3.nb  
...
def streamplot(axes,x, y, u, v, density=1, linewidth=None, color=None,
               cmap=None, norm=None, arrowstyle='-|>',
    ...
    line_kw = {}
    arrow_kw = dict(arrowstyle=arrowstyle)
    ...
    for t in trajectories:
        ...
        if isinstance(linewidth, np.ndarray):
            ...
            arrow_kw['mutation_scale'] = np.max([10*line_widths[n],20.0])
...
X = np.loadtxt('velocity_x_weak_9.2_figure_3.txt', comments='%')[:,0] #exported from weak_acid_2D.mph
Y = np.loadtxt('velocity_x_weak_9.2_figure_3.txt', comments='%')[:,1]
U = np.loadtxt('velocity_x_weak_9.2_figure_3.txt', comments='%')[:,2:]
V = np.loadtxt('velocity_y_weak_9.2_figure_3.txt', comments='%')[:,2:] #exported from weak_acid_2D.mph
X_sol = np.loadtxt('sol_vel_x_weak_9.2_figure_3.txt', comments='%')[:,0] #exported from weak_acid_2D.mph
Y_sol = np.loadtxt('sol_vel_x_weak_9.2_figure_3.txt', comments='%')[:,1]
U_sol = np.loadtxt('sol_vel_x_weak_9.2_figure_3.txt', comments='%')[:,2:]
V_sol = np.loadtxt('sol_vel_y_weak_9.2_figure_3.txt', comments='%')[:,2:] #exported from weak_acid_2D.mph


# In[4]:


X_sol = X_sol[~np.isnan(U_sol[:,0])]
Y_sol = Y_sol[~np.isnan(U_sol[:,0])]
V_sol = V_sol[~np.isnan(U_sol[:,0])]
U_sol = U_sol[~np.isnan(U_sol[:,0])]
X = X[~np.isnan(U[:,0])]
Y = Y[~np.isnan(U[:,0])]
V = V[~np.isnan(U[:,0])]
U = U[~np.isnan(U[:,0])]
...
X_tot = np.concatenate((X, X_sol))
Y_tot = np.concatenate((Y, Y_sol))
U_tot = np.concatenate((U, U_sol))
V_tot = np.concatenate((V, V_sol))
speed = np.sqrt(np.square(U_tot) + np.square(V_tot))
max_speed = 0.10300846114205513
...
y_grid = np.linspace(0,2,2000)
X_grid, Y_grid = np.meshgrid(x_grid, y_grid)
grid = np.array([X_grid.flatten(), Y_grid.flatten()]).T


# In[7]:


mesh = np.array([X_tot,Y_tot]).T


# In[8]:


X_start = np.linspace(0.2,0.8,9)
Y_start = np.linspace(0,2,10)
X_start_grid, Y_start_grid = np.meshgrid(X_start, Y_start)
start_grid = np.array([X_start_grid.flatten(), Y_start_grid.flatten()]).T
...
def speedplot(n):
    speed_intp = interpolate.griddata(mesh, speed[:,n], grid, method="cubic").reshape([2000,2000])
    U_grid = interpolate.griddata(mesh, U_tot[:,n], grid, method="cubic").reshape([2000,2000])
    V_grid = interpolate.griddata(mesh, V_tot[:,n], grid, method="cubic").reshape([2000,2000])
    U_grid[speed_intp<0.001]=0
    V_grid[speed_intp<0.001]=0
    speed_intp[speed_intp<0.001]=0.001
    lw = ma.log(speed_intp/max_speed) + 10
    lw.filled(0)
    fig, ax = plt.subplots(figsize=(50,20))
    ax.set_xlim([0,1])
    ax.set_ylim([0,2])
    #pcm = ax.pcolormesh(X_grid, Y_grid, speed_intp, shading='gouraud', cmap='YlOrBr', norm=colors.LogNorm(vmin=0.0001, vmax=max_speed))
    streamplot(ax,X_grid, Y_grid, U_grid, V_grid, linewidth=lw, density=(2,6), start_points=start_grid,color='red', maxlength = 0.1, minlength=0.01)
    ax.axis('off')
    #fig.colorbar(pcm, location='bottom')
    plt.savefig('./velocity_weak_9.2_figure_3.png', bbox_inches='tight', pad_inches=0,dpi=300, transparent=True)
    #plt.close(fig)
    speed_intp=[]
    V_grid=[]
    U_grid=[]
...
img1 = plt.imread('velocity_weak_9.2_figure_3.png')
img2 = plt.imread('deform_weak_9.2_figure_3.png')
fig, ax =  plt.subplots(figsize=(50,20))
ax.imshow(img2,origin='upper',aspect='equal',extent=(0,800,0,320))
ax.imshow(img1, origin='upper', aspect='equal',extent=(0,800,0,320))
ax.set_xticks(np.arange(0,801,step=160), ['0','0.2','0.4','0.6','0.8','1'])
ax.set_yticks(np.arange(0,321,step=160), ['0', '1', '771'])
ax.tick_params(axis="x",direction="inout", length=60, width=6, labelsize=120)
ax.tick_params(axis="y",direction="inout", length=60, width=6, labelsize=120)
for axis in  ['top','bottom','left','right']:
    ax.spines[axis].set_linewidth(6)
ax.spines[['right', 'top']].set_visible(False)
plt.xlim(288,512)
plt.title(f'{9.2:.2f}'+r'$\tau$', fontsize=120)
plt.savefig('final_t_9.2_weak_9.2_figure_3.png', dpi=300, pad_inches=0, bbox_inches='tight')
#plt.close(fig)
